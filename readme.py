...
class ReadMe(object):
    ...
    def __subscribeLink(self, fileName:str, url:str=None):
        ...
        if url:
            ...
            link += " [原始链接](https://raw.githubusercontent.com/C9LG/DNS-Blocklists/main/rules/%s) |"%(fileName)
        
        for i in range(1, len(self.proxyList)):
            proxy = self.proxyList[i]
            if proxy.startswith("https://gcore.jsdelivr.net/"):
                link += " [加速链接%d](%s/C9LG/DNS-Blocklists@main/rules/%s) |"%(i, proxy, fileName)
            else:
                link += " [加速链接%d](%s/https://raw.githubusercontent.com/C9LG/DNS-Blocklists/main/rules/%s) |"%(i, proxy, fileName)
    ...
    def regenerate(self):
        ...
        with open(self.filename, 'a') as f:
            ...
            f.write("3. 规则1’、2’为规则1、2的 Lite 版，仅针对国内域名拦截，体积较小（如添加完整规则报错数量限制，请尝试 Lite 规则）\n")
            f.write("4. 已对 jsdelivr 缓存进行主动刷新，但 jsdelivr 加速链接仍存在一定延时\n")
            f.write("\n")
            tmp = "| 规则 | 原始链接 |"
            for i in range(1, len(self.proxyList)):
                tmp += " 加速链接%d |"%(i)
